...
public class CatGridViewAdapter extends BaseAdapter {
    ...
    public CatGridViewAdapter(Context context, List<Catas> mDatas, int curIndex, int pageSize) {
...
//        if (convertView == null) {
            convertView = inflater.inflate(R.layout.cat_item_gridview, parent, false);
...
//        }
        ...
        viewHolder.tv.setText(mDatas.get(pos).getSubCataName());
        String bannerImg;
        if(MyConstant.PIC_DPI2==1){
            bannerImg = (mDatas.get(pos).getIconMidAndroid()).trim();
        }else if(MyConstant.PIC_DPI2==2){
            bannerImg = (mDatas.get(pos).getIconBigAndroid()).trim();
        }else{
            bannerImg = (mDatas.get(pos).getIconBigAndroid()).trim();
        }
