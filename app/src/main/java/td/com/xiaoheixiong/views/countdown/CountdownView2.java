package td.com.xiaoheixiong.views.countdown;
...
public class CountdownView2 extends View {
    ...
    //秒字体背景
    private Paint mSecondTimeTextBgPaint;
    private int mSecondTimeBgColor;

    //秒字体颜色
    private Paint mSecondTimeTextPaint;
    private int mSecondTimeTextColor;
    ...
    private int mSuffixTextColor;//冒号颜色
    private float mSuffixTextSize;//冒号字体大小
    ...
    public CountdownView2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSecondTimeBgColor= ta.getColor(R.styleable.CountdownView_timeBgColor, 0xFFff3646);
        mSecondTimeTextColor= ta.getColor(R.styleable.CountdownView_timeBgColor, 0xFFff0000);
    ...
    private void initPaint() {
        ...
        mSecondTimeTextBgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mSecondTimeTextBgPaint.setStyle(Paint.Style.FILL);
        mSecondTimeTextBgPaint.setColor(mSecondTimeBgColor);



        mSecondTimeTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mSecondTimeTextPaint.setColor(mSecondTimeTextColor);
        mSecondTimeTextPaint.setTextAlign(Paint.Align.CENTER);
        mSecondTimeTextPaint.setTextSize(mTimeTextSize);
        if (isTimeTextBold) {
            mSecondTimeTextPaint.setFakeBoldText(true);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isHideTimeBackground) {
            ...
            if (isShowSecond) {
                // draw second text
                canvas.drawText(formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mSecondTimeTextPaint);
                if (mSuffixSecondTextWidth > 0) {
                    // draw second suffix
            ...
            if (isShowSecond) {
                ...
                canvas.drawRoundRect(mSecondBgRectF, mTimeBgRadius, mTimeBgRadius, mSecondTimeTextBgPaint);
                if (isShowTimeBgDivisionLine) {
        ...
        mCustomCountDownTimer = new CustomCountDownTimer(millisecond, countDownInterval) {
            ...
            public void onFinish() {
                ...
                if (null != mOnCountdownEndListener) {
                    mOnCountdownEndListener.onEnd(CountdownView2.this);
                }
            }
        };
    ...
    public interface OnCountdownEndListener {
        void onEnd(CountdownView2 cv);
    }

    public interface OnCountdownIntervalListener {
        void onInterval(CountdownView2 cv, long remainTime);
    }
